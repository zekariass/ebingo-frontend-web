...
export function StartGameButton({ disabled, selectedCards, fee }: StartGameButtonProps) {
  ...
  const { room } = useRoomStore()
  const { game: {gameId, userSelectedCardsIds, started, joinedPlayers} } = useGameStore()
  ...
  return (
    ...
    //           Cancel
    //         </Button>
    //         <Button className="flex-1" onClick={handleConfirmAndPay} disabled={!canAfford || isProcessing}>
    //           {isProcessing ? (
    //             <>
    //               <div className="animate-spin rounded-full h-4 w-4 border-2 border-white border-t-transparent mr-2" />
    //               Processing...
