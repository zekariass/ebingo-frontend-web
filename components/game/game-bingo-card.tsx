...
export function GameBingoCard({ card }: GameBingoCardProps) {
  ...
  const handleCellClick = async (rowIndex: number, colIndex: number, number: number) => {
    ...
    if (hasClaimed) {
      toast({
        title: "Game Over",
        description: "You have already claimed BINGO and cannot mark more numbers.",
      })
      return
    }
    ...
    if (!isCalled) {
      toast({
        ...
        description: `${BINGO_LETTERS[colIndex]} ${number} has not been called yet.`,
      })
      return
    }
    ...
    if (newMarkedCells.has(cellKey)) {
      ...
      toast({
        ...
        description: `${BINGO_LETTERS[colIndex]} ${number}`,
      })
    } else {
      newMarkedCells.add(cellKey)
      console.log("[v0] Cell marked:", cellKey)
      ...
      try {
        ...
        toast({
          variant: "destructive",
          title: "Error",
          description: "Failed to mark number. Please try again.",
        })
      }
    }
  ...
  const checkWinningPattern = () => {
    const patternName = currentPattern?.name?.toLowerCase() || "line"

    const checkHorizontalLines = () => {
      for (let row = 0; row < 5; row++) {
        let hasLine = true
        for (let col = 0; col < 5; col++) {
          const cellKey = `${row}-${col}`
    ...
    const checkFourCorners = () => {
      ...
      return corners.every((corner) => markedCells.has(corner))
    }
    ...
    switch (patternName) {
      ...
      case "line + four corners":
      case "line_four_corners":
        return (checkHorizontalLines() || checkVerticalLines() || checkDiagonalLines()) && checkFourCorners()
      default:
        return checkHorizontalLines() || checkVerticalLines() || checkDiagonalLines()
    }
  }
  ...
  const handleBingoClaim = async () => {
    ...
    try {
      ...
      if (response.success && response.verified) {
        ...
        toast({
          title: "🎉 BINGO! 🎉",
          description: `Congratulations! You won $${response.prize}!`,
        })
      } else {
        console.log("❌ Bingo claim rejected")
        toast({
          variant: "destructive",
          title: "Invalid BINGO",
          description: "Your claim was not verified. Please check your card pattern.",
        })
      }
    } catch (error) {
      console.error("[v0] Error claiming bingo:", error)
  ...
  return (
    <div className="bg-card rounded-lg border p-3">
          ...
          row.map((number, colIndex) => {
            ...
            return (
              <button
                ...
                disabled={hasClaimed}
                className={cn(
                  "aspect-square flex items-center justify-center rounded text-xs sm:text-sm font-medium border-2 transition-all duration-200 cursor-pointer active:scale-95 touch-manipulation",
                  isFree
                    ? "bg-yellow-500 text-white border-yellow-600 cursor-pointer"
                    : isMarked
                      ? "bg-blue-500 text-white border-blue-600 shadow-md"
                      : isCalled
                        ? "bg-green-100 text-green-800 border-green-300 hover:bg-green-200 ring-2 ring-green-400 ring-opacity-50"
                        : hasClaimed
                          ? "bg-muted text-muted-foreground border-border opacity-50 cursor-not-allowed"
                          : "bg-muted text-muted-foreground border-border hover:bg-muted/80",
                )}
      ...
      <Button
        size="sm"
        className={cn(
          "w-full transition-all duration-300 font-bold text-white shadow-lg text-xs sm:text-sm py-2 sm:py-3",
          hasWinningPattern && !hasClaimed && !isClaimingBingo
            ? "bg-red-600 hover:bg-red-700 active:bg-red-800 border-2 border-red-400 hover:shadow-xl active:scale-95 animate-pulse"
            : hasClaimed
              ? "bg-green-600 border-2 border-green-400 cursor-not-allowed"
              : "bg-gray-400 hover:bg-gray-500 cursor-not-allowed opacity-50",
        )}
        onClick={handleBingoClaim}
        disabled={!hasWinningPattern || hasClaimed || isClaimingBingo}
      >
        {isClaimingBingo ? "Claiming..." : hasClaimed ? "✓ BINGO CLAIMED" : "BINGO"}
      </Button>
    </div>
